...
class Database :
    ...
    #	@valet	/ instance of valet itself to interact with main object
 	#	@file	/ str > file location of database object to open
    #
    #######################################################################
    def __init__ ( self, valet=None, file=None, ) : 
	    try :
	        self.conn = None
	        self.cursor = None
    ...
    def get(self, table, columns, condition=None, limit=None ):

        query = "SELECT {0} FROM {1}{2};".format( columns, table, ( 'WHERE %s' % condition if condition else ''))        
